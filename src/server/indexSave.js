...
const resolvers = {
    Query: {
        ...
        eventos: async () => {
            const { data, error } = await supabase.from('eventos').select('*,categorias(*)')
...
async function startApolloServer() {
    ...
    app.listen(4000, () => {
        ...
        console.log(`GraphQL disponible en http://localhost:4000${server.graphqlPath}`);
    })
...
/* 
Podemos dar alias definiendo el alias seguido de dos puntos, ejemplo Alias:eventos. Esto funciona tanto para atributos del objeto como para cualquier query
query{
  Eventos:eventos {
    id
    titulo
